...
namespace IO.Swagger.Model {
  ...
  public class DocumentCreationInfo {
    ...
    /// </summary>
    /// <value>A publicly accessible url to which Adobe Sign will do an HTTP GET operation every time there is a new agreement event. HTTP authentication is supported using standard embedded syntax - i.e. http://username:password@your.server.com/path/to/file. Adobe Sign can also ping your system using HTTP PUT with the final signed PDF. Please contact support@echosign.com if you wish to use this option.</value>
    ...
    /// </summary>
    /// <value>Specifies the type of signature you would like to request - written or e-signature. The possible values are ESIGN or WRITTEN</value>
    ...
    /// <summary>
    /// The name of a library document that is available to the sender. If multiple library documents having the same name exist, the latest one is selected. This can be used to build integrations that allow the contents of the document to be changed, in the future, without any code changes
    /// </summary>
    /// <value>The name of a library document that is available to the sender. If multiple library documents having the same name exist, the latest one is selected. This can be used to build integrations that allow the contents of the document to be changed, in the future, without any code changes</value>
    ...
    /// <summary>
    /// Optional parameter that sets how often you want to send reminders to the recipients. The possible values are DAILY_UNTIL_SIGNED, WEEKDAILY_UNTIL_SIGNED, EVERY_OTHER_DAY_UNTIL_SIGNED, EVERY_THIRD_DAY_UNTIL_SIGNED, EVERY_FIFTH_DAY_UNTIL_SIGNED, WEEKLY_UNTIL_SIGNED
    /// </summary>
    /// <value>Optional parameter that sets how often you want to send reminders to the recipients. The possible values are DAILY_UNTIL_SIGNED, WEEKDAILY_UNTIL_SIGNED, EVERY_OTHER_DAY_UNTIL_SIGNED, EVERY_THIRD_DAY_UNTIL_SIGNED, EVERY_FIFTH_DAY_UNTIL_SIGNED, WEEKLY_UNTIL_SIGNED</value>
    ...
    /// <summary>
    /// Selects the workflow you would like to use - whether the sender needs to sign before the recipient, after the recipient, or not at all. The possible values for this variable are SENDER_SIGNATURE_NOT_REQUIRED, SENDER_SIGNS_LAST, or SENDER_SIGNS_FIRST
    /// </summary>
    /// <value>Selects the workflow you would like to use - whether the sender needs to sign before the recipient, after the recipient, or not at all. The possible values for this variable are SENDER_SIGNATURE_NOT_REQUIRED, SENDER_SIGNS_LAST, or SENDER_SIGNS_FIRST</value>
    ...
    /// <summary>
    /// A list of documents associated with this agreement. The documentIDs are returned in the transient document creation API
    /// </summary>
    /// <value>A list of documents associated with this agreement. The documentIDs are returned in the transient document creation API</value>
    ...
    /// </summary>
    /// <value>A list of one or more recipients. For regular (non-MegaSign) documents, there is no limit on the number of electronic signatures in a single document. Written signatures are limited to four per document. This limit includes the sender if the signature of the sender is also required</value>
